...
uint32_t g_local_src_ip = IP_STR_COVER_UINT(192, 168, 1, 2);

static char *mac2string_print(uint8_t *mac)
{
	static char buf[sizeof("HH:HH:HH:HH:HH:HH") + 1];
...
static int eth_packet_encap(uint8_t *packet, uint8_t *src_mac, uint8_t *dst_mac, uint16_t ether_type)
{
	struct rte_ether_hdr *eth_packet = (struct rte_ether_hdr *)packet;
...
/*
	|arp
*/
static int arp_packet_encap(uint8_t *packet, uint8_t *src_mac, uint8_t *dst_mac,
	uint32_t src_ip, uint32_t dst_ip, uint16_t opcode)
{
	struct rte_arp_hdr *arp_packet = (struct rte_arp_hdr *)packet;
...
static struct rte_mbuf * arp_reply_packet_encap(struct rte_mempool *mp, int port_id,
	struct rte_ether_addr *dst_mac, uint32_t src_ip, uint32_t dst_ip)
{
	...
	eth_packet_encap((uint8_t *)eth_hdr, (uint8_t *)&src_mac, (uint8_t *)dst_mac, RTE_ETHER_TYPE_ARP);

	/* arp */
	struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_arp_hdr *, sizeof(struct rte_ether_hdr));
	arp_packet_encap((uint8_t *)arp_hdr, (uint8_t *)&src_mac, (uint8_t *)dst_mac, src_ip, dst_ip, RTE_ARP_OP_REPLY);
	printf("send arp reply:[MAC:%s IP:%s]",  mac2string_print(arp_hdr->arp_data.arp_sha.addr_bytes),
		inet_ntoa(*(struct in_addr *)&arp_hdr->arp_data.arp_sip));
	printf("send arp reply:[MAC:%s IP:%s]\n", mac2string_print(arp_hdr->arp_data.arp_tha.addr_bytes),
	   ...
	   void *arg)
{
	...
	rte_eth_macaddr_get(g_port_id, &mac_addr);
	...
	for (i = 1; i <= 254; i++) {
		...
		packet = rte_pktmbuf_mtod(mbuf, uint8_t *);

		/* 构造以太网头 */
		eth_packet_encap(packet, (uint8_t *)&mac_addr, (uint8_t *)arp_dst_mac, RTE_ETHER_TYPE_ARP);

		/*构造arp报文*/
		arp_packet_encap(packet + sizeof(struct rte_ether_hdr),
			(uint8_t *)&mac_addr, (uint8_t *)arp_dst_mac, g_local_src_ip, IP_STR_COVER_UINT(192, 168, 1, i), RTE_ARP_OP_REQUEST);

		/* 发送报文 */
		rte_eth_tx_burst(g_port_id, 0, &packet, 1);
...
/* 设定网口的收发包队列 */
static void dpdk_eth_dev_init(struct rte_mempool * mp)
{
	uint16_t ports;
...
static int arp_packet_parse(struct rte_mbuf *rx_packet, struct rte_ether_addr *src_mac,
		struct rte_ether_addr *dst_mac, uint32_t *src_ip ,uint32_t *dst_ip)
{
	struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(rx_packet, struct rte_arp_hdr *,
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		for (i = 0; i < recv_nums; i++) {
			...
			if (ntohs(ether_hdr->ether_type) == RTE_ETHER_TYPE_ARP) {
				arp_packet_parse(rx_pkts[i], &src_mac, &dst_mac, &src_ip, &dst_ip);
				printf("arp packet:[MAC:%s IP:%s]--->", mac2string_print(src_mac.addr_bytes), inet_ntoa(*(struct in_addr *)&src_ip));
				printf("arp packet:[MAC:%s IP:%s]\n", mac2string_print(dst_mac.addr_bytes), inet_ntoa(*(struct in_addr *)&dst_ip));
				struct rte_arp_hdr *arp_hdr = rte_pktmbuf_mtod_offset(rx_pkts[i],
				...
				} else if (ntohs(arp_hdr->arp_opcode) == RTE_ARP_OP_REPLY) {
					/* 接收arp响应报文 */
